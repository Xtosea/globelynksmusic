export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/comment" && request.method === "POST") {
      ...
      let commentData;
      try { commentData = JSON.parse(body); } 
      catch (err) { return new Response("Invalid JSON", { status: 400 }); }

      meta.comments = meta.comments || [];
      meta.comments.push({ text: commentData.comment, date: new Date().toISOString() });

      await env.R2_BUCKET.put(`meta/${id}.json`, JSON.stringify(meta));

      return new Response(JSON.stringify({ comments: meta.comments }), {
        headers: { "Content-Type": "application/json", ...corsHeaders }
      });
    }
